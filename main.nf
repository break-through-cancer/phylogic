...
with open(maf_in, "r", newline="") as f:
    ...
    # ------------------------------------------------------------
    # Funcotator-specific MAF header fixes for PhylogicNDT
    #
    # Funcotator writes standard MAF column names, but this older
    # PhylogicNDT code expects a few slightly different names.
    # This renames the header only; row values stay unchanged.
    # ------------------------------------------------------------
    ...
    header = [funcotator_to_phylogic.get(c, c) for c in header]
